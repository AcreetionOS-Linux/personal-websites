#!/usr/bin/env bash
# build.sh — build the HarrisOS ISO from upstream AcreetionOS
#
...
echo "→ building HarrisOS (harrisos) — this takes a while…"
(cd "$SRC" && sudo ./build.sh "$ID" unofficial)
trap - EXIT
restore_overlays

echo ""
echo "🎉 done — your HarrisOS ISO is in $SRC/../ISO/$ID/"
